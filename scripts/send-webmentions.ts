 ...
 *
 * Usage: node scripts/send-webmentions.ts [--dry-run] [--force]
 *
 * Options:
 ...
 */
function readSentCache(): Record<string, SentRecord> {
 ...
 */
function writeSentCache(data: Record<string, SentRecord>): void {
 ...
 */
function findBlogPosts(): BlogPost[] {
 ...
 */
function extractLinks(html: string, _sourceUrl: string): string[] {
...
async function discoverEndpoint(targetUrl: string): Promise<string | null> {
  try {
    ...
    console.warn(
      `  [!] Error discovering endpoint for ${targetUrl}: ${(error as Error).message}`,
    );
    return null;
  }
}
...
async function sendWebmention(
  ...
  try {
    ...
    if (response.ok || response.status === 201 || response.status === 202) {
      ...
      console.warn(
        `  [!] Failed (${response.status}): ${source} -> ${target}`,
      );
      return false;
    }
  } catch (error) {
    console.warn(`  [!] Error sending: ${(error as Error).message}`);
    return false;
  }
}
