...
const withAssets: boolean = args.includes("--with-assets");
const title: string | undefined = args.filter(
  (arg) => !arg.startsWith("--"),
)[0];
...
if (withAssets) {
  ...
  const postDir: string = join(postsDir, slug);
  ...
  const filePath: string = join(postDir, "index.md");
  writeFileSync(filePath, frontmatter);
  console.log(`Created: ${filePath}`);
  console.log(`Add images to: ${postDir}/`);
} else {
  // Create simple flat post
  const filePath: string = join(postsDir, `${slug}.md`);
