const TRAILING_MARKDOWN_TITLE_RE = /^(.+?)\s+(?:"([^"]*)"|'([^']*)'|\(([^)]*)\)|&quot;([^&]*)&quot;)$/;

export function parseMarkdownImageInner(inner) {
  if (!inner) {
    return { path: "", title: "" };
  }
  ...
  if (withTitle) {
    ...
    title = withTitle[2] || withTitle[3] || withTitle[4] || withTitle[5] || "";
  }
...
export function normalizeImageReference(imagePath) {
  return parseMarkdownImageInner(imagePath).path;
}

export function extractFirstImageReference(content) {
