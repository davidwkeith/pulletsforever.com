...
function makeCardId(title: string, pageInputPath: string): string {
...
async function renderHtmlToPng(htmlString: string): Promise<Buffer> {
...
function resolveImagePath(
  ...
  const normalizedImagePath = normalizeImageReference(imagePath as string);
...
async function generateSocialCard(
  ...
  if (hasImage) {
    try {
      ...
      console.error(`Error processing image "${imagePath}":`, (err as Error).message);
...
export default function (eleventyConfig: EleventyConfig): void {
    ...
    for (const [cardId, { title, imagePath }] of pendingCards) {
      ...
      promises.push(
        generateSocialCard(title, imagePath, outputPath).catch((err) => {
          console.error(
            `Error generating social image for "${title}":`,
  ...
  eleventyConfig.addFilter("firstImage", function (content: string) {
